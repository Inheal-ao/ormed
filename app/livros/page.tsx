...
import { BookMarked, Loader2, ExternalLink, BookOpen, Download, X } from "lucide-react";
import { usePublicData } from "@/lib/use-public-data";
import { Paginated, BookItem } from "@/lib/admin-types";

export default function LivrosPage() {
  const { data, loading } = usePublicData<Paginated<BookItem>>("/books?limit=100");
    ...
    <div className="pt-28 pb-16 min-h-screen">
      <section className="bg-angola-navy text-white py-16">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <h1 className="text-4xl md:text-5xl font-bold">Livros</h1>
          <p className="text-gray-300 text-lg mt-4 max-w-2xl">
            Livros e obras de referência da comunidade médica.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        {loading ? (
          ...
          <div className="bg-gray-50 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-600">Os livros serão publicados em breve.</p>
          </div>
        ) : (
                ...
                <div className="aspect-[3/4] bg-angola-navy relative overflow-hidden">
                  {b.coverImage?.url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <BookMarked className="w-12 h-12 text-white/30" />
                    </div>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="font-bold text-gray-900 line-clamp-1">{b.title}</h3>
                  <p className="text-sm text-gray-500 mb-3 line-clamp-1">{b.author || "—"}</p>
                  {b.pdf?.url ? (
                    <button type="button" onClick={() => setActive(b)} className="w-full flex items-center justify-center gap-1.5 bg-angola-gold text-angola-navy text-sm font-semibold py-2 rounded-lg hover:brightness-95">
                      <BookOpen className="w-4 h-4" />Ler
                    </button>
                  ) : b.externalLink ? (
                    <a href={b.externalLink} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-1.5 bg-angola-navy text-white text-sm font-semibold py-2 rounded-lg hover:brightness-110">
                      <ExternalLink className="w-4 h-4" />Aceder
                    </a>
                  ) : null}
                </div>
              </div>
