...
import { Pencil, Trash2, Loader2, Eye, EyeOff, Briefcase, MapPin } from "lucide-react";
import { api } from "@/lib/api";
import { Paginated, JobItem } from "@/lib/admin-types";
import { PageHeader } from "@/components/admin/admin-ui";

export default function VagasListPage() {
  const [data, setData] = useState<Paginated<JobItem> | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      setData(await api.get<Paginated<JobItem>>("/jobs/admin/all?limit=50", true));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Eliminar esta vaga?")) return;
    setDeleting(id);
    try {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Oportunidades de emprego publicadas no site."
        action={{ label: "Nova vaga", href: "/admin/vagas/novo" }}
      />
      {loading ? (
        ...
        <p className="text-gray-500 text-center py-16">Ainda não há vagas.</p>
      ) : (
          ...
          {data.items.map((item) => (
            <div key={item._id} className="flex items-center gap-4 p-4">
              <div className="w-11 h-11 rounded-lg bg-angola-navy/5 flex items-center justify-center shrink-0">
                <Briefcase className="w-5 h-5 text-angola-navy" />
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/admin/vagas/${item._id}`} className="p-2 text-gray-500 hover:text-angola-navy hover:bg-gray-100 rounded-lg" aria-label="Editar">
