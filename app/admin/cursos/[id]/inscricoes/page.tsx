...
import { EventRegistration, CourseItem } from "@/lib/admin-types";
import { BackLink } from "@/components/admin/admin-ui";
import { printTable } from "@/lib/print";

const STATUS_LABEL: Record<string, string> = { pending: "Pendente", validated: "Validada", rejected: "Rejeitada" };
...
export default function CursoInscricoesPage() {
  ...
  const load = async () => {
    ...
    try {
      const [c, regs] = await Promise.all([
        api.get<CourseItem>(`/courses/admin/${id}`, true),
  ...
  const setStatus = async (regId: string, status: string) => {
    ...
    setItems((prev) => prev.map((r) => (r._id === regId ? { ...r, status: status as EventRegistration["status"] } : r)));
  };
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `inscricoes-${course?.slug ?? id}.csv`;
      a.click();
  ...
  return (
    <div>
      <BackLink href={`/admin/cursos/${id}`} label="Voltar ao curso" />
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Total: {items.length}{course?.capacity ? ` / ${course.capacity} vagas` : ""}
          </p>
        </div>
        {items.length > 0 && (
          <div className="flex gap-2 shrink-0">
            ...
            <button type="button" onClick={exportCsv} disabled={exporting} className="inline-flex items-center gap-2 bg-angola-navy text-white font-semibold px-4 py-2 rounded-lg hover:brightness-110">
