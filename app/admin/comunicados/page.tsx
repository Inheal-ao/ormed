...
import { Pencil, Trash2, Loader2, Eye, EyeOff, Megaphone } from "lucide-react";
...
export default function ComunicadosListPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setData(await api.get<Paginated<AnnouncementItem>>("/announcements/admin/all?limit=50", true));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <PageHeader
        title="Comunicados"
        description="Comunicados oficiais da Ordem."
        action={{ label: "Novo comunicado", href: "/admin/comunicados/novo" }}
      />
      {loading ? (
        ...
        <p className="text-gray-500 text-center py-16">Ainda não há comunicados.</p>
      ) : (
          ...
          {data.items.map((item) => (
            <div key={item._id} className="flex items-center gap-4 p-4">
              <div className="w-11 h-11 rounded-lg bg-angola-navy/5 flex items-center justify-center shrink-0">
                <Megaphone className="w-5 h-5 text-angola-navy" />
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/admin/comunicados/${item._id}`} className="p-2 text-gray-500 hover:text-angola-navy hover:bg-gray-100 rounded-lg" aria-label="Editar">
