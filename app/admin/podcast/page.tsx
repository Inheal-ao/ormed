...
import { Pencil, Trash2, Loader2, Eye, EyeOff, Mic } from "lucide-react";
import { api } from "@/lib/api";
import { Paginated, PodcastItem } from "@/lib/admin-types";
import { PageHeader } from "@/components/admin/admin-ui";

export default function PodcastListPage() {
  const [data, setData] = useState<Paginated<PodcastItem> | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      setData(await api.get<Paginated<PodcastItem>>("/podcast/admin/all?limit=50", true));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <PageHeader
        title="Podcast"
        description="Episódios em vídeo (YouTube)."
        action={{ label: "Novo episódio", href: "/admin/podcast/novo" }}
      />
      {loading ? (
        ...
        <p className="text-gray-500 text-center py-16">Ainda não há episódios.</p>
      ) : (
          ...
          {data.items.map((item) => (
            <div key={item._id} className="flex items-center gap-4 p-4">
              <div className="w-12 h-12 rounded-lg bg-angola-navy/5 flex items-center justify-center shrink-0">
                <Mic className="w-5 h-5 text-angola-navy" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-900 truncate">{item.title}</p>
                <p className="text-sm text-gray-500">{item.episode}</p>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/admin/podcast/${item._id}`} className="p-2 text-gray-500 hover:text-angola-navy hover:bg-gray-100 rounded-lg" aria-label="Editar">
