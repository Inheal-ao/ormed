  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <PageHeader title="Programas de Ensino para os Internatos" description="Currículos e programas de formação dos internatos, por colégio." />
...
function ProgramaForm({ programa, colleges, isColegio, defaultCollege, onClose, onSaved }: {
  ...
  const [f, setF] = useState({ college: programa?.college ?? defaultCollege ?? "", title: programa?.title ?? "", ano: programa?.ano ?? "", description: programa?.description ?? "" });
  ...
  const submit = async () => {
    ...
    try {
      ...
      fd.append("title", f.title); fd.append("ano", f.ano); fd.append("description", f.description);
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-md w-full p-6 my-8" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900">{programa ? "Editar programa" : "Novo programa"}</h3>
        ...
        <div className="space-y-3">
          ...
          <input className={inputClass} placeholder="Título do programa *" value={f.title} onChange={(e) => set("title", e.target.value)} />
          <input className={inputClass} placeholder="Ano / Nível (ex: 1º ano)" value={f.ano} onChange={(e) => set("ano", e.target.value)} />
          <textarea className={`${inputClass} min-h-[90px]`} placeholder="Descrição / conteúdo programático" value={f.description} onChange={(e) => set("description", e.target.value)} />
          <label className={`flex items-center gap-2 border-2 border-dashed rounded-lg px-3 py-2.5 text-sm cursor-pointer ${file ? "border-green-400 bg-green-50" : "border-gray-300 hover:border-angola-gold"}`}>
            <Upload className="w-4 h-4" /> {file ? file.name : (programa?.document ? "Substituir PDF do programa" : "Anexar PDF do programa")}
