...
export default function NoticiasListPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get<Paginated<NewsItem>>("/news/admin/all?limit=50", true);
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Eliminar esta notícia? Esta ação não pode ser revertida.")) return;
    setDeleting(id);
    try {
  ...
  return (
    <div>
      <PageHeader
        title="Notícias"
        description="Gerir as notícias publicadas no site."
      ...
      {loading ? (
        ...
        <p className="text-gray-500 text-center py-16">Ainda não há notícias.</p>
      ) : (
            ...
            <div key={item._id} className="flex items-center gap-4 p-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <Link
                  ...
                  className="p-2 text-gray-500 hover:text-angola-navy hover:bg-gray-100 rounded-lg"
                  aria-label="Editar"
                >
