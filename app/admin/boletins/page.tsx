...
export default function BoletinsListPage() {
  const [data, setData] = useState<Paginated<BulletinItem> | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      setData(await api.get<Paginated<BulletinItem>>("/bulletins/admin/all?limit=50", true));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Eliminar este boletim?")) return;
    setDeleting(id);
    try {
  ...
  return (
    <div>
      <PageHeader
        title="Boletins"
        description="Boletins informativos em PDF."
        action={{ label: "Novo boletim", href: "/admin/boletins/novo" }}
      />
      {loading ? (
        ...
        <p className="text-gray-500 text-center py-16">Ainda não há boletins.</p>
      ) : (
            ...
            <div key={item._id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="flex items-center gap-1 mt-2">
                  <Link href={`/admin/boletins/${item._id}`} className="flex-1 text-center text-xs py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50">
